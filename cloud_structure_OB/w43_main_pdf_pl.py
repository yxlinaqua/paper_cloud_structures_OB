...
ax1.errorbar(0.5 * (norm_n[:-1] + norm_n[1:]), norm_p[1:],
             yerr=((np.sqrt(test0[0])) / np.sum(test0[0][1:] * s))[[test0[1] > (cutoff / meanvalue)]][1:], capsize=1.0,
             capthick=1, linewidth=1.0, ls='none', color='black')
ax1.plot(np.log(test[2]), test[3] / np.sum(test[0][1:] * s), 'r--', linewidth=1.5,
         label='power-law fit from optimal column density')
...
#calculate the mass percentage of PL1 and PL2
x_set1 = out1.params['p1'].value
y_set = np.exp(x_set) * meanvalue
y_set1 = np.exp(x_set1) * meanvalue
y3 = y2 * meanvalue
mass_excess_per = np.sum(y3[y3 > y_set]) / np.sum(
    y3)  # *2.8*con.m_p.cgs/(1.9891e+33*u.g)*(((1.5*distance/206265.).to(u.cm))**2)
mass_excess_per1 = np.sum(y3[y3 > y_set1]) / np.sum(y3)
fitx = np.linspace(-1.2, 2.1, 1000)
y_ln = 1.0 / (ro_fit.lognormal.sigma * np.sqrt(2 * np.pi)) * np.exp(
    -1 * (fitx - ro_fit.lognormal.mu) ** 2 / (2 * ro_fit.lognormal.sigma ** 2))
...
-1 * ro_fit.power_law.alpha, ro_fit.power_law.sigma, -1 * ro1_fit.power_law.alpha, ro1_fit.power_law.sigma,
-1 * ro2_fit.power_law.alpha, ro2_fit.power_law.sigma), size='large', color='black')
