...
for i, leaf in enumerate(d.leaves):
    if not (NH2_mean[
                i] < threshold):  # and eff_radius[i]**2 > 0.15):# and not(NH2_mean[i] < 2.1e22 and eff_radius[i]**2 <0.08):
...
for i, leaf in enumerate(d.leaves):
    ...
    mass_corrected_leaf = np.append(mass_corrected_leaf, fluxes_corrected * 2.8 * con.m_p.cgs / (1.9891e+33 * u.g) * (
        ((1.5 * distance / 206265.).to(u.cm)) ** 2))
    mass_leaf = np.append(mass_leaf, cat[leaf.idx]['flux'] * 2.8 * con.m_p.cgs / (1.9891e+33 * u.g) * (
        ((1.5 * distance / 206265.).to(u.cm)) ** 2))
    eff_radius_leaf = np.append(eff_radius_leaf,
                                np.sqrt(cat[leaf.idx]['area_exact'] / np.pi) * distance / 206265. * u.pc)
...
ax3.plot(eff_radius_leaf[NH2_mean > threshold], mass_leaf[NH2_mean > threshold], 's', color='red', label='mass',
         markeredgecolor='none', alpha=0.5)
ax3.plot(eff_radius_leaf[NH2_mean > threshold], mass_corrected_leaf[NH2_mean > threshold], 's', color='purple',
         label='corrected mass', markeredgecolor='none', alpha=0.5)
...
(ar, br) = polyfit(np.log10(eff_radius_leaf[NH2_mean > 10 * 8.0e20 / 2.35 * 5.]),
                   np.log10(mass_leaf[NH2_mean > threshold]), 1)
xr = polyval([ar, br], np.log10(r.value))

ax3.plot(r, np.power(10, xr), 'r--')

# linear regression###
(ar1, br1) = polyfit(np.log10(eff_radius_leaf[NH2_mean > 10 * 8.0e20 / 2.35 * 5.]),
                     np.log10(mass_corrected_leaf[NH2_mean > 10 * 8.0e20 / 2.35 * 5.]), 1)
